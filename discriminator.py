...
class Discriminator(object):
    ...
    def _add_placeholders(self):
        ...
        # encoder part
        self._target_batch = tf.placeholder(tf.int32, [hps.batch_size.value* hps.max_enc_sen_num.value, hps.max_enc_seq_len.value], name='enc_batch')
        #self._target_lens = tf.placeholder(tf.int32, [hps.batch_size.value* hps.max_enc_sen_num.value], name='enc_lens')

        self._dec_batch = tf.placeholder(tf.int32, [hps.batch_size.value * hps.max_enc_sen_num.value, hps.max_enc_seq_len.value], name='enc_batch')
        self._dec_lens = tf.placeholder(tf.int32, [hps.batch_size.value * hps.max_enc_sen_num.value], name='enc_lens')
        #self._enc_sen_lens = tf.placeholder(tf.int32, [hps.batch_size.value * hps.], name='enc_sen_lens')
        ...
        self.label = tf.placeholder(tf.float32, [hps.batch_size.value * hps.max_enc_sen_num.value, hps.max_enc_seq_len.value], name="positive_negtive")

        #self._target_batch = tf.placeholder(tf.int32,
        #                                    [hps.batch_size.value* hps.max_enc_sen_num.value],
        #                                    name='target_batch')
    ...
    def _build_model(self):
        ...
        with tf.variable_scope('discriminator'):
            ...
            with tf.variable_scope('output_projection'):
                w = tf.get_variable('w_output', [hps.hidden_dim.value, vsize], dtype=tf.float32,
                                    initializer=self.trunc_norm_init)
            ...
            with tf.variable_scope('decoder'):
                ...
                self._dec_in_state = cell.zero_state(FLAGS.batch_size* hps.max_enc_sen_num.value, tf.float32)
            ...
            self.out_put = tf.reshape(self.out_put, [hps.batch_size.value, hps.max_enc_sen_num.value])'''
