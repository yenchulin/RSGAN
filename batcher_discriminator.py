...
class Example(object):
  ...
  def __init__(self, review, label, vocab, hps):
    ...
    for i in range(len(article_sens)):
        ...
        if len(article_sen_words) > hps.max_enc_seq_len.value:
            article_sen_words = article_sen_words[:hps.max_enc_seq_len.value]
    ...
    self.dec_input, self.target = self.get_dec_inp_targ_seqs(abs_ids, hps.max_enc_sen_num.value, hps.max_enc_seq_len.value,
                                                              start_decoding,
...
class Batch(object):
  ...
  def init_decoder_seq(self, example_list, hps):
      ...
      for ex in example_list:
          ex.pad_decoder_inp_targ(hps.max_enc_seq_len.value, hps.max_enc_sen_num.value, self.pad_id)

      # Initialize the numpy arrays.
      # Note: our decoder inputs and targets must be the same length for each batch (second dimension = max_dec_steps) because we do not use a dynamic_rnn for decoding. However I believe this is possible, or will soon be possible, with Tensorflow 1.0, in which case it may be best to upgrade to that.
      self.dec_batch = np.zeros((hps.batch_size.value, hps.max_enc_sen_num.value, hps.max_enc_seq_len.value), dtype=np.int32)
      self.target_batch = np.zeros((hps.batch_size.value, hps.max_enc_sen_num.value, hps.max_enc_seq_len.value), dtype=np.int32)
      self.dec_padding_mask = np.zeros((hps.batch_size.value * hps.max_enc_sen_num.value, hps.max_enc_seq_len.value),
                                       dtype=np.float32)
      self.labels = np.zeros((hps.batch_size.value, hps.max_enc_sen_num.value, hps.max_enc_seq_len.value), dtype=np.int32)
      self.dec_sen_lens = np.zeros((hps.batch_size.value, hps.max_enc_sen_num.value), dtype=np.int32)
      self.dec_lens = np.zeros((hps.batch_size.value), dtype=np.int32)
      self.review_sentenc_orig = []

      for i, ex in enumerate(example_list):
          #self.new_review_text = []
          self.labels[i]=np.array([[ex.label for k in range(hps.max_enc_seq_len.value) ] for j in range(hps.max_enc_sen_num.value)])
      ...
      self.target_batch = np.reshape(self.target_batch,
                                     [hps.batch_size.value * hps.max_enc_sen_num.value, hps.max_enc_seq_len.value])
      ...
      self.dec_batch = np.reshape(self.dec_batch, [hps.batch_size.value * hps.max_enc_sen_num.value, hps.max_enc_seq_len.value])
      self.dec_sen_lens = np.reshape(self.dec_sen_lens, [hps.batch_size.value * hps.max_enc_sen_num.value])
      self.labels = np.reshape(self.labels, [hps.batch_size.value * hps.max_enc_sen_num.value, hps.max_enc_seq_len.value])
...
class DisBatcher(object):
    ...
    def create_batches(self, mode="train", shuffleis=True):
        ...
        for i in range(0, num_batches):
            batch = []
            if mode == 'train':
                batch += (self.train_queue[i*self._hps.batch_size.value:i*self._hps.batch_size.value + self._hps.batch_size.value])
            elif mode == 'test':
                batch += (self.test_queue[i*self._hps.batch_size.value:i*self._hps.batch_size.value + self._hps.batch_size.value])
