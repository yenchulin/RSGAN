...
class Example(object):
  ...
  def __init__(self, review, vocab, hps, input=None):
    ...
    if input !=None:
        ...
        for i in range(len(abstract_sentences)):
            if i >= hps.max_dec_sen_num.value:
                abstract_words = abstract_words[:hps.max_dec_sen_num.value]
                break
            abstract_sen = abstract_sentences[i]
            abstract_sen_words = abstract_sen.split()
            if len(abstract_sen_words) > hps.max_dec_steps.value:
                abstract_sen_words = abstract_sen_words[:hps.max_dec_steps.value]
        ...
        for i in range(len(abstract_sentences)):
            if i >= hps.max_dec_sen_num.value:
                abstract_words = abstract_words[:hps.max_dec_sen_num.value]
                break
            abstract_sen = abstract_sentences[i] # string
            abstract_sen_words = abstract_sen.split() # list of strings (words)
            if len(abstract_sen_words) > hps.max_dec_steps.value:
                abstract_sen_words = abstract_sen_words[:hps.max_dec_steps.value]
            abstract_words.append(abstract_sen_words)
    ...
    self.dec_input, self.target = self.get_dec_inp_targ_seqs(abs_ids, hps.max_dec_sen_num.value, hps.max_dec_steps.value,
  ...
  def get_dec_inp_targ_seqs(self, sequence, max_sen_num, max_len, start_id, stop_id):
    ...
    # Add start_id for each input sequence
    for i in range(len(inps)):
        inps[i] = [start_id] + inps[i][:]
        if len(inps[i]) > max_len:
            inps[i] = inps[i][:max_len]

    # Add stop_id for each target sequence
    for i in range(len(targets)):
        if len(targets[i]) >= max_len:
            targets[i] = targets[i][:max_len - 1]  # leave a space for "end token"
...
class Batch(object):
  ...
  def __init__(self, example_list, hps, vocab):
    ...
    Args:
       example_list: List of Example objects, shape = (batch_size, )
       hps: hyperparameters
       vocab: Vocabulary object
    """
...
class GenBatcher(object):
    ...
    def create_batch(self, mode="train", shuffleis=True):
        ...
        if mode == "train":
            num_batches = len(self.train_queue) // self._hps.batch_size.value
            if shuffleis:
                shuffle(self.train_queue)
        elif mode == 'test':
            num_batches = len(self.test_queue) // self._hps.batch_size.value

        for i in range(0, num_batches):
            batch = []
            start = i * self._hps.batch_size.value
            end = (i + 1) * self._hps.batch_size.value

            if mode == 'train':
                batch += self.train_queue[start: end]
            elif mode == 'test':
                batch += self.test_queue[start: end]
